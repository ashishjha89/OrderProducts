...
- **GraphQL Federation**: Apollo Router (supergraph gateway).
...
- **Purpose**: Supergraph gateway — composes `product-service` and `order-service` subgraphs into a single GraphQL API
- **Endpoint**: `http://localhost:4000/`
- **Sandbox UI**: `http://localhost:4000/` (enabled via `router.yaml`)
- **Health check**: `http://localhost:4000/health` (internal port 8088)
- **Config**: `router.yaml`, loaded via Docker volume mount
- **Supergraph SDL**: `supergraph.graphql`, generated by `rover` CLI and loaded at router startup

### Supergraph files
| File                     | Purpose                                                                               |
|--------------------------|---------------------------------------------------------------------------------------|
| `supergraph.yaml`        | `rover` config — maps subgraph names to routing URLs and SDL files                    |
| `product-schema.graphql` | Clean SDL for product-service subgraph (input to `rover`)                             |
| `order-schema.graphql`   | Clean SDL for order-service subgraph (input to `rover`)                               |
| `supergraph.graphql`     | Composed supergraph SDL (output of `rover`, loaded by router)                         |
| `router.yaml`            | Apollo Router config: sandbox, introspection, listen address, local-dev URL overrides |

### Regenerating the supergraph after schema changes
```bash
cd infrastructure
...
### Running Apollo Router for local development (services on host, not Docker)
The specific ports below are **only required when the Apollo Router runs in Docker while the services run on your host**. `router.yaml` has hardcoded `override_subgraph_url` entries pointing to `host.docker.internal:8082` and `host.docker.internal:8084`. If you use different ports, update those entries in `router.yaml` to match.

GraphQL works on each service independently regardless of which port it runs on. The ports only matter for the router-to-host routing.

```bash
# Terminal 1 — product-service (port 8082 only required so the router can reach it)
cd product-service && mvn spring-boot:run -Dspring-boot.run.jvmArguments="-Dserver.port=8082"

# Terminal 2 — order-service (port 8084 only required so the router can reach it)
cd order-service && ./mvnw spring-boot:run -Dspring-boot.run.jvmArguments="-Dserver.port=8084"

# Start only the router (--no-deps skips the Docker container versions of the services)
cd infrastructure && docker compose up apollo-router --no-deps -d
```

### Example federated query (run against the router)
```graphql
mutation {
  placeOrder(input: {
    orderLineItems: [{ skuCode: "samsung-s10", price: 999.99, quantity: 2 }]
  }) {
    orderId
    orderNumber
    lineItems {
...
```
The router routes the mutation to **order-service**, then calls **product-service** `_entities` to hydrate the `Product` fields, and merges the result into a single response.
